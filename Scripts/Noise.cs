...
namespace ProceduralMapGenerator.Scripts
{
    public class Noise
    {
		private int seed; //the seed this noise was initialized with, for informational purpose
		private int subSeed; //the seed that could be changed without re-building permutation. Never bigger permCount.
		...
		public float Fractal(float x, float y, float size, int iterations = -1, float detail = 0.5f, float turbulence = 0, int type = 2, bool native = true)
		{
			...
			if (iterations < 0) iterations = (int)Math.Log(size, 2) + 1; //+1 max size iteration
